...
#define HEATER_PWM_TIM_CLK          (SystemCoreClock)
#define HEATER_PWM_TIM_PRESCALER    (HEATER_PWM_TIM_CLK / HEATER_PWM_FREQUENCY / HEATER_PWM_PERIOD) - 1
...
uint32_t HW_GetTipTemperatureMicrovolt(void)
{
    ...
    adcValueIntegrated >>= 4;
    uint32_t uV = (uint32_t)( ( adcValueIntegrated * ADC_REF_VOLTAGE_MICROVOLT) / ADC_QUANTISATION_LEVELS);
    return  uV / TIP_FRONTEND_GAIN;
}
...
//    uint64_t adcValueIntegrated = 0;
//    _ADC_ReadChannel(CHANNEL_T_AMB);
//    for( uint8_t i = 0; i < 16; i++)
//    {
//        // Read the ADC value
//       adcValueIntegrated += _ADC_ReadChannel(CHANNEL_T_AMB);
//    }
//    adcValueIntegrated >>= 4;
//
//    uint32_t mV = (uint32_t)( ( adcValueIntegrated * ADC_REF_VOLTAGE_MILLIVOLT) / ADC_QUANTISATION_LEVELS);
//
//
//    int16_t temp = (10 * (mV - MCP_MILLIVOLT_V0C)) / MCP_TEMPERATURE_COEFFICIENT; // in .1 °C
//    return (temp + 5) / 10; // in °C
	// TODO Sth is terribly wrong in reading this channel - to be investigated
    return 25;
}
...
{
    return SysTick->CNT / SYSTICK_DIV_TO_MILLISECOND;
}
...
{
    uint32_t start = HW_GetSystemTimestamp();
    while (HW_GetSystemTimestamp() - start < milliseconds);
}
...
/**
 * @brief Initialize the system time. A 64-bit timer is used to count the CPU cycles.
*/
void _SYSTICK_Init(void)
{
    // Risc-V QingKe V4 SysTick
    // Control Register
    // Enable SystemCounter,
    // SystemCoreClock as source
    // Continue counting when CMP is reached
    SysTick->CTLR = 0x05;
...
void Heater_Init(void)
{
	...
	TIM_TimeBaseInitStructure.TIM_Period = HEATER_PWM_PERIOD ; // 16-bit
